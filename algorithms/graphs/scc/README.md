...
The reason why this algorithm works is that the transpose of a graph has the same strongly connected components as the original graph. This means that if we run a DFS on the transpose of a graph, we will get a depth-first forest where each tree is a strongly connected component of the original graph. By running a DFS on the transpose of $G$ in decreasing order of the finish times of $G$, we are guaranteed to explore all vertices in a strongly connected component before moving on to the next one, since the connecting edges between components in $G^T$ are the reverse of the connecting edges in $G$, meaning that any connecting edge in $G^T$ will be a back edge in $G$ pointing to a vertex that has already been explored. This is why the algorithm returns a depth-first forest where each tree is a strongly connected component of $G$.
