...
void fillTable (int ***table, const char *a, const char *b, int la, int lb){
  *table = calloc(la+1, sizeof(int*)); 
  for (int i=0; i<=la; ++i) (*table)[i] = calloc(lb+1, sizeof(int)); 
  for (int i=1; i<=la; ++i)
    for (int j=1; j<=lb; ++j){
...
void getLcs (int **table, char *a, char *b, int x, int y, char **str, int z){
  if (x == 0 || y == 0) return;
  if (a[x-1] == b[y-1]) {
    (*str)[z] = a[x-1]; return getLcs(table, a, b, x-1, y-1, str, z-1);  
  }
  if (table[x][y-1] > table[x-1][y]) return getLcs(table, a, b, x, y-1, str, z); 
...
char *lcs(char *a, char *b){
  ...
  char *str = malloc((clslen+1)*sizeof(char));
