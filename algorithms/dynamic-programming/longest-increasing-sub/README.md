...
To find an actual LIS, we can keep an extra array $prev$ of size $n$. In order to keep things efficient at this stage, we should keep indexes instead of actual values in both the $dp$ and $prev$ arrays. We will update the $prev$ array as we update the $dp$ array. Each time we update the $dp$ array, we also update the $prev$ array by setting the value of $prev[i]$ to the value stored in $dp[i-1]$ (i.e. the previous element of the LIS ending at the $i$ th element of the array). We use the index -1 to denote that there is no previous element. When we are done updating the $dp$ array, the last element of the $dp$ array will be the last element of the LIS, and the value of $prev[i]$ at this index will be the index of the previous element of the LIS ending at the $i$ th element of the array. So, we can reconstruct the LIS by iterating backwards from the index of the last element of the $dp$ array and taking the value of $prev[i]$ at each step, until we reach an index -1. This reconstruction takes $O(n)$ time.

The overall time complexity of this solution is in $O(n \log n)$, since iterating over the array takes $O(n)$ time and each binary search takes $O(\log n)$ time. The space complexity is in $O(n)$, since we need an array $dp$ of size $n$ and an array $prev$ of size $n$.
