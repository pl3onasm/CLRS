...
To find an actual LIS, we can keep track of the previous element of the LIS at each step in an extra array $prev$ of size $n$, and at the end we can then iterate backwards from the last element of the LIS to find the actual LIS. We update this $prev$ array each time we append a new element to the $dp$ array with the value of the current last element of the $dp$ array before we append the new element.
